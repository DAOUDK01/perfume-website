...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!metricsRes.ok)
          throw new Error(metricsData?.error || "Failed to load dashboard");
        if (cancelled) return;
  ...
  const stats = [
    ...
    },
    {
      name: "Revenue",
      value: metrics
        ? formatMoney(Number(metrics.revenue || 0), currency)
        : "—",
      change: "",
      icon: BarChart3,
    ...
    },
    {
      ...
      value: metrics
        ? formatMoney(Number(metrics.monthlyRevenue || 0), currency)
        : "—",
      change: "This month",
      icon: BarChart3,
      color: "from-gray-500",
    },
    {
      name: "Yearly Revenue",
      value: metrics
        ? formatMoney(Number(metrics.yearlyRevenue || 0), currency)
        : "—",
      change: "This year",
      icon: BarChart3,
      color: "from-gray-700",
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-amber-50 border border-amber-200 text-sm text-amber-900">
          <div>
            <span className="font-medium">
              {metrics.newOrdersCount} new order
              {metrics.newOrdersCount > 1 ? "s" : ""}
            </span>
        ...
        {stats.map((stat, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-6">
                <p className="text-3xl font-serif font-light text-gray-900 mb-1">
                  {stat.value}
                </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-2xl p-8 flex flex-col lg:h-[560px]">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-8">
          ...
          <div className="space-y-3">
            <Link
              ...
              className="w-full p-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors block"
            >
              ➕ Add Product
            </Link>
            <Link
